...
from keras.layers import (Layer, Input, Reshape, Rescaling, Flatten, Dense, Dropout, TimeDistributed, Lambda, Conv1D,
                          BatchNormalization, Activation, Add, GRU, LSTM, LayerNormalization, MultiHeadAttention)
...
def AttentionEncoder(inputs, num_heads, head_size, dropout, units_dim):
...
def build_rnn_attention():
    input_sig = Input(shape=(12800, 15))
    sig = Rescaling(scale=1.0/6065.3467)(input_sig)
    sig = Reshape((50, 256, 15))(sig)
    sig = TimeDistributed(Flatten())(sig)

    sig = TimeDistributed(Dense(1024, activation="relu"))(sig)
    sig = TimeDistributed(Dropout(0.2))(sig)
    sig = TimeDistributed(Dense(512, activation="relu"))(sig)
    sig = TimeDistributed(Dropout(0.2))(sig)
    
    sig = GRU(256, return_sequences=True)(sig)  #sig = LSTM(256, return_sequences=True)(sig)
    sig = GRU(64, return_sequences=True)(sig)  #sig = LSTM(64, return_sequences=True)(sig)
    
    sig = AttentionEncoder(sig, num_heads=1, head_size=128, dropout=0.2, units_dim=64)
    ...
    model._name = "RNN_Attention_Model"

    return model

with strategy.scope():
    rnn_attention_model = build_rnn_attention()

rnn_attention_model.summary()

rnn_attention_history = rnn_attention_model.fit(X_train,
                                                    ...
                                                    verbose = 1,
                                                    callbacks = [ModelCheckpoint("rnn_attention_fault_detr_v1.h5",
                                                                                    verbose = 1,
                                                                                    monitor = "val_loss",
                                                                                    save_best_only = True,
...
np.save("rnn_attention_fault_detr_v1_history.npy", rnn_attention_history.history)
rnn_attention_model_history = np.load("rnn_attention_fault_detr_v1_history.npy", allow_pickle="TRUE").item()

rnn_attention_model.load_weights("rnn_attention_fault_detr_v1.h5")

test_metrics = rnn_attention_model.evaluate(X_test, [y_test[:,:23], y_test[:,23:]])
test_metrics
...
test_eval(rnn_attention_model, rnn_attention_model_history)